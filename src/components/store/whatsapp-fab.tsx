...
export async function WhatsAppFab() {
  ...
  const link = buildWhatsAppLink(
    ...
    `مرحبًا، من ${settings.shopName}\nعندي استفسار عن أحد القطع أو طلب خاص.`
  );
  if (!link) return null;
  ...
  return (
    <a
      ...
      className="fixed bottom-5 end-5 z-50 grid h-14 w-14 place-items-center rounded-full bg-primary text-white shadow-lg shadow-primary/30 transition-transform hover:scale-110 no-print"
    >
